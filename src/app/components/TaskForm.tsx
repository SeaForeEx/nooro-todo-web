...
export default function TaskForm({ onClose, initial }: { onClose: () => void; initial?: Task }) {
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (initial?.id) await updateTask(initial.id, { title, color });
    ...
    <div className="absolute top-[291px] left-1/2 transform -translate-x-1/2 w-[736px] h-[358px] flex flex-col gap-[48px] p-6 opacity-100">
      <form onSubmit={submit} className="flex flex-col gap-6">
        ...
        <div className="flex gap-3 w-full mt-6">
            <button
                className="flex flex-shrink-0 items-center justify-center gap-2 w-[736px] h-[52px] rounded-lg bg-[#1E6F9F] text-[#F2F2F2] text-center font-inter font-bold text-[14px] leading-[140%] tracking-[0%] opacity-100 hover:bg-[#155a75]"
            >
                {initial?.id ? (
                    <>
